...
- **バフ・デバフシステム:** 状態効果の管理と視覚的表現

### 11.3. バフ・デバフアーキテクチャ

- **状態効果管理:** `Character`クラスの`buffs`/`debuffs`配列による一元管理
- **期限管理:** `updateBuffsAndDebuffs()`による自動期限切れ処理
- **視覚表現:** HUDアイコンによる状態効果の視覚的フィードバック
- **効果適用:** 攻撃力・防御力・移動速度等の乗算係数による効果適用
- **状態種別:**
  - **攻撃力増強 (attackEnhancement):** 攻撃力倍率向上
  - **防御力増強 (defenseEnhancement):** 防御力倍率向上
  - **毒状態 (poison):** 継続ダメージ効果
  - **移動速度減少 (slowness):** 移動速度低下
